...
export default function BlogFilter({ posts }: BlogFilterProps) {
  ...
  // Get all unique categories sorted by popularity (most posts first)
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            >
              {isCategoryFilterExpanded ? (
                ...
                <>
                  Show categories ({allCategories.length})
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isCategoryFilterExpanded && (
            <div className="flex flex-wrap gap-2">
              {allCategories.map(category => {
                const postCount = categoryCounts.get(category) || 0;
                return (
                  <button
                    key={category}
                    ...
                    title={`${postCount} post${postCount !== 1 ? 's' : ''} in this category`}
                  >
                    {category} ({postCount})
                    {selectedCategories.includes(category) && (
